...
if( ! empty( $fields ) ) {
	...
	<div class="wpfep-fields">
		...
		foreach( $fields as $field ) {
			...
			$element_classes = ( empty ($field['element_classes'])) ? '' : ' ' . $field['element_classes'];

			/* output the field */
			wpfep_field( $field, $classes, $tab[ 'id' ], get_current_user_id(), $element_classes );
...
*/
function wpfep_field( $field, $classes, $tab_id, $user_id, $element_classes ) {
	...
	if( in_array( $field[ 'id' ], $reserved_ids ) ) {
		...
		$current_field_value = maybe_unserialize(get_user_meta( get_current_user_id(), $field[ 'id' ], true ));
		$current_field_array = (is_array($current_field_value)) ? $current_field_value : array();
	...
	?>
	<select class="wpfep-select<?php echo esc_attr($element_classes); ?>" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>]" id="<?php echo $field[ 'id' ]; ?>">
	...
	<textarea class="wpfep-textarea<?php echo esc_attr($element_classes); ?>" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>]" rows="<?php echo apply_filters( 'wpfep_textarea_rows', '5', $field[ 'id' ] ); ?>" cols="50" id="<?php echo esc_attr( $field[ 'id' ] ); ?>" class="regular-text"><?php echo esc_textarea( $current_field_value ); ?></textarea>
	...
	?>
	<input class="wpfep-checkbox<?php echo esc_attr($element_classes); ?>" type="checkbox" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>]" id="<?php echo esc_attr( $field[ 'id' ] ); ?>" value="1" <?php checked( $current_field_value, '1' ); ?> />
	<?php
	...
	?>
	<input class="regular-text wpfep-text wpfep-email<?php echo esc_attr($element_classes); ?>" type="email" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>]" id="<?php echo esc_attr( $field[ 'id' ] ); ?>" value="<?php echo esc_attr( $current_field_value ); ?>" />
	...
	/* geosuggest place field */
	case 'place':
	?>
	<input class="regular-text wpfep-text wpfep-place<?php echo esc_attr($element_classes); ?>" name="<?php echo esc_attr( $tab_id ); ?>[__discard]" data-geo="label"
	value="<?php echo esc_attr( $current_field_array['label'] ); ?>" type="text" />
	<input type="hidden" data-geo="label" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>][label]" value="<?php echo esc_attr( $current_field_array['label'] ); ?>" />
	<input type="hidden" data-geo="lat" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>][lat]" value="<?php echo esc_attr( $current_field_array['lat'] ); ?>" />
	<input type="hidden" data-geo="lng" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>][lng]" value="<?php echo esc_attr( $current_field_array['lng'] ); ?>" />
	...
	?>
	<input class="regular-text wpfep-default<?php echo esc_attr($element_classes); ?>" type="text" name="<?php echo esc_attr( $tab_id ); ?>[<?php echo $field[ 'id' ]; ?>]" id="<?php echo esc_attr( $field[ 'id' ] ); ?>" value="<?php echo esc_attr( $current_field_value ); ?>" />
	<?php
